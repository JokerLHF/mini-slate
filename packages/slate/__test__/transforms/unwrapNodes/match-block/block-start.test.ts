...
describe('transfroms.unwrapNodes', () => {
  ...
  beforeAll(() => {
    ...
    editor.selection = { anchor: { path: [0,0,0], offset: 0 }, focus: { path: [0,1,0], offset: 0 } };
  });

  it('block', () => {
    Transforms.unwrapNodes(editor, { match: n => n.type === 'a' });
    expect(editor.children).toStrictEqual(output);
    expect(editor.selection).toStrictEqual({ anchor: { path: [0,0], offset: 0 }, focus: { path: [1,0], offset: 0 } });
  })
})
